...
export default function Watch() {
  ...
  const [upcoming, setUpcoming] = useState(null); // 10 sn içinde başlayacak
  const [active, setActive] = useState(null);     // şu anda içindeyiz

  // Efekt durumları: ses & karartma ve “ne zamana kadar sürsün”
  const [dim, setDim] = useState(false);
  const [vol, setVol] = useState(1);
  const [dimUntil, setDimUntil] = useState(null);   // saniye
  const [volUntil, setVolUntil] = useState(null);   // saniye
  ...
  // Renkler (timeline)
  const colorClass = (t) =>
    t === "violence" ? "bg-red-500 focus:ring-red-400"
    : t === "fear"   ? "bg-yellow-500 focus:ring-yellow-400"
  ...
  const handleLowerVolume = () => {
    const target = upcoming || active;
    if (!target) return;
    setVol(0.3);                 // hemen etkilesin
    setVolUntil(target.end + 0.01); // risk bitimine kadar
    setShowBanner(false);
  };
    ...
    <div className="space-y-6">
      ...
      {showBanner && upcoming && (
        <div className="rounded-xl bg-yellow-500/20 border border-yellow-500/40 text-yellow-200 px-4 py-3 flex items-center gap-3">
          ⚠️ <b className="mr-2">{upcoming.type}</b> sahnesi <b>{Math.ceil(upcoming.start - now)} sn</b> içinde
          <div className="flex items-center gap-2 ml-auto">
            <button onClick={handleSkip} className="px-3 py-1 rounded-lg bg-blue-600 hover:bg-blue-500 text-white">
              Atla
            </button>
            <button onClick={handleLowerVolume} className="px-3 py-1 rounded-lg bg-gray-700 hover:bg-gray-600">
              Sesi Kıs
            </button>
            <button onClick={handleDim} className="px-3 py-1 rounded-lg bg-gray-700 hover:bg-gray-600">
              Karart
            </button>
          ...
          {spans.map((s, i) => {
            ...
            return (
              <button
                ...
                className={`absolute -translate-x-1/2 top-0 h-3 w-1 ${colorClass(s.type)} focus:outline-none focus:ring-2 focus:ring-offset-1`}
